...
func GetBybitCloseProfit_Loss(apiKey string, secret string) (*Statment, error) {
	queryString := "category=linear"
	timestamp := time.Now().UnixNano() / int64(time.Millisecond)
	accSignature := GenerateBybitSignature(apiKey, secret, 50000, timestamp, queryString)
	finalURL := BybitAPIEndpoint + "/v5/position/closed-pnl?" + queryString
...
func GetOrderHistory(apiKey string, secret string) (*OrderHistory, error) {
	...
	timestamp := time.Now().UnixNano() / int64(time.Millisecond)
	accSignature := GenerateBybitSignature(apiKey, secret, 50000, timestamp, queryString)
	finalURL := BybitAPIEndpoint + "/v5/order/history?" + queryString
