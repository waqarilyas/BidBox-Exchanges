...
func GetBinanceAccountOrderHistory(apiKey string, secret string) (*[]Order, error) {
	timestamp := time.Now().UnixNano() / int64(time.Millisecond)
	...
	accSignature := GenerateBinanceSignature(params, secret)
	finalURL := BinanceAPIEndpoint + "/fapi/v1/allOrders?timestamp=" + strconv.FormatInt(timestamp, 10) + "&signature=" + accSignature
